...
| `gerador.ts`, `site-claro.ts`, `site-editorial.ts`, `nichos/*.ts`, `catalogo.ts` | puro | gerador de sites: planilha → leads, nicho, slug e os dois motores de página — claro (pet, saúde, aulas) e editorial (beleza, tatuagem, fitness, automotivo); o conteúdo de cada nicho mora em `nichos/`, e `catalogo.ts` é a tabela de serviços que o `seed:agenda` também usa |
